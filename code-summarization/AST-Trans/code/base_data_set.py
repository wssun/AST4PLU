...
class BaseASTDataSet(data.Dataset):
    ...
    def convert_ast_to_tensor(self, ast_seq, tokenizer):
        source_tokens = tokenizer.tokenize(ast_seq)[:self.max_src_len - 2]
        source_tokens = [tokenizer.cls_token] + source_tokens + [tokenizer.sep_token]
        source_ids = tokenizer.convert_tokens_to_ids(source_tokens)
        padding_length = self.max_src_len - len(source_ids)
        source_ids += [tokenizer.pad_token_id] * padding_length

        return torch.tensor(source_ids, dtype=torch.long)

    def convert_nl_to_tensor(self, nl, tokenizer):
        target_tokens = tokenizer.tokenize(nl)[:self.max_tgt_len - 2]
        target_tokens = [tokenizer.cls_token] + target_tokens + [tokenizer.sep_token]

        target_ids = tokenizer.convert_tokens_to_ids(target_tokens)
        target_mask = [1] * len(target_ids)
        padding_length = self.max_tgt_len - len(target_ids)
