...
class MyAst:
    ...
    @staticmethod
    def collect_seq_and_save(root_nodes, output_file, seq_type, idx, nls):
        parallel = joblib.Parallel(n_jobs=n_jobs)
        if seq_type == 'sbt':
        ...
        with open(output_file, 'w') as f:
            for line_index, line in enumerate(seqs):
                ...
                js = {'idx':idx[line_index], 'ast':line, 'nl':nls[line_index]}
                f.write(json.dumps(js) + '\n')

    @staticmethod
    def collect_matrices_and_save(root_nodes, output_matrices_file, output_pot_file, idx, nls):
        parallel = joblib.Parallel(n_jobs=n_jobs)
    ...
    def __process(ast_json, split_leaf=False, max_size=-1):
        ...
        for i in range(node_num):
            ...
            node.label = node_attr['label']
            children = node_attr['children']
            if len(children) == 1 and children[0] == -1:   # leaf node
