...
def read_examples(filename, mode='token'):
...
def main():
    ...
    if args.do_train:
        ...
        train_data = TensorDataset(source_ids, source_ids2, all_target_ids, all_target_mask)
        ...
        for epoch in range(args.num_train_epochs):
            ...
            for batch in bar:
                ...
                source_ids, source_ids2, target_ids, target_mask = batch
                loss,_,_ = model(source_ids=source_ids, source_ids2=source_ids2,
            ...
            if args.save_every > 0 and epoch % args.save_every == 0:
                ...
                if 'dev_bleu' in dev_dataset:
                    ...
                    eval_data = TensorDataset(source_ids, source_ids2)
                ...
                for batch in eval_dataloader:
                    batch = tuple(t.to(device) for t in batch)
                    source_ids, source_ids2 = batch
                    with torch.no_grad():
                        preds = model(source_ids=source_ids, source_ids2=source_ids2)
                        for pred in preds:
                            t=pred[0].cpu().numpy()
                            t=list(t)
                            if 0 in t:
