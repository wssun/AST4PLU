...
class MyAst:
    ...
    @staticmethod
    def collect_seq_and_save(root_nodes, output_file, seq_type, idx):
        parallel = joblib.Parallel(n_jobs=n_jobs)
        if seq_type == 'sbt':
        ...
        with open(output_file, 'w') as f:
            for line_index, line in enumerate(seqs):
                ...
                js = {'idx':idx[line_index], 'ast':line}
                f.write(json.dumps(js) + '\n')

    @staticmethod
    def collect_matrices_and_save(root_nodes, output_matrices_file, output_pot_file, idx):
        parallel = joblib.Parallel(n_jobs=n_jobs)
